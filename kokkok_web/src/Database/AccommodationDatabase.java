...
public class AccommodationDatabase extends CustomDatabase
{ 
	String []columnList = {"", "acc_no", "name", "address", "latitude", "longitude", "phone", "intro", "sub_intro"};
	
	public HashMap<String, Object> getAccommodationInfo(String item_id)
	{
		connectSQL();
		
		String query = "select * from accommodation where acc_no=?";
...
//		connectSQL();
//		
//		String query = "select *, (6371 * acos(cos(radians(?)) * cos( radians(latitude)) * cos(radians(longitude) - radians(?)) + sin(radians(?)) * sin( radians(latitude)))) as distance from accommodation having distance < ?";
//		
...
//	}
	...
	public String getAround(String acc_no, double latitude, double longitude, double distance)
	{
		connectSQL();
		...
		try
		{
			...
			while( rs.next() )
			{
				if( acc_no != null )
				{
					if( !acc_no.equals(rs.getString(1)) )
					{
	...
	public ArrayList<HashMap<String, Object>> getDB(String local)
	{
		...
		String query = "select * from accommodation where acc_no like ? '%'";
...
//		
//		String query = "select count(acc_no) from accommodation where acc_no like ? '%'";
//		int totalPage = 0;
//		
//		try
//		{
...
//			local = local.substring(0, 2);
//		
//		String query = "select * from accommodation where acc_no like ? '%' limit ?, ?";
//	
//		try
//		{
//			pstmt = con.prepareStatement(query);
