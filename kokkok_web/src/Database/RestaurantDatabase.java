...
public class RestaurantDatabase extends CustomDatabase
{ 
	String []columnList = {"", "rst_no", "name", "address", "latitude", "longitude", "phone", "intro", "time", "day_off", "menu"};
	
	public HashMap<String, Object> getRestaurantInfo(String item_id)
	{
		connectSQL();
		
		String query = "select * from restaurant where rst_no=?";
...
//		connectSQL();
//		
//		String query = "select *, (6371 * acos(cos(radians(?)) * cos( radians(latitude)) * cos(radians(longitude) - radians(?)) + sin(radians(?)) * sin( radians(latitude)))) as distance from restaurant having distance < ?";
//		
...
//	}
	...
	public String getAround(String rst_no, double latitude, double longitude, double distance)
	{
		connectSQL();
		...
		try
		{
			...
			while( rs.next() )
			{
				if( rst_no != null )
				{
					if( !rst_no.equals(rs.getString(1)) )
					{
	...
	public ArrayList<HashMap<String, Object>> getDB(String local)
	{
		...
		String query = "select * from restaurant where rst_no like ? '%'";
...
//		
//		String query = "select count(rst_no) from restaurant where rst_no like ? '%'";
//		int totalPage = 0;
//		
//		try
//		{
...
//			local = local.substring(0, 2);
//		
//		String query = "select * from restaurant where rst_no like ? '%' limit ?, ?";
//	
//		try
//		{
//			pstmt = con.prepareStatement(query);
