...
public class ShoppingDatabase extends CustomDatabase
{ 
	String []columnList = {"", "shop_no", "name", "phone", "address", "latitude", "longitude", "intro", "time"};
	
	public HashMap<String, Object> getShoppingInfo(String item_id)
	{
		connectSQL();
		
		String query = "select * from shopping where shop_no=?";
...
//		connectSQL();
//		
//		String query = "select *, (6371 * acos(cos(radians(?)) * cos( radians(latitude)) * cos(radians(longitude) - radians(?)) + sin(radians(?)) * sin( radians(latitude)))) as distance from shopping having distance < ?";
//		
...
//	}
	...
	public String getAround(String shop_no, double latitude, double longitude, double distance)
	{
		connectSQL();
		...
		try
		{
			...
			while( rs.next() )
			{
				if( shop_no != null )
				{
					if( !shop_no.equals(rs.getString(1)) )
					{
	...
	public ArrayList<HashMap<String, Object>> getDB(String local)
	{
		...
		String query = "select * from shopping where shop_no like ? '%'";
...
//		
//		String query = "select count(shop_no) from shopping where shop_no like ? '%'";
//		int totalPage = 0;
//		
//		try
//		{
...
//			local = local.substring(0, 2);
//		
//		String query = "select * from shopping where shop_no like ? '%' limit ?, ?";
//	
//		try
//		{
//			pstmt = con.prepareStatement(query);
