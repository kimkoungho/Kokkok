...
public class WifiDatabase extends CustomDatabase
{ 
	String []columnList = {"", "wifi_no", "name", "latitude", "longitude"};
	
	public HashMap<String, Object> getWifiInfo(String item_id)
	{
		connectSQL();
		
		String query = "select * from wifi where wifi_no=?";
...
//		connectSQL();
//		
//		String query = "select *, (6371 * acos(cos(radians(?)) * cos( radians(latitude)) * cos(radians(longitude) - radians(?)) + sin(radians(?)) * sin( radians(latitude)))) as distance from wifi having distance < ?";
//		
...
//	}
	...
	public String getAround(String wifi_no, double latitude, double longitude, double distance)
	{
		connectSQL();
		...
		try
		{
			...
			while( rs.next() )
			{
				if( wifi_no != null )
				{
					if( !wifi_no.equals(rs.getString(1)) )
					{
	...
	public ArrayList<HashMap<String, Object>> getDB(String local)
	{
		...
		String query = "select * from wifi where wifi_no like ? '%'";
...
//		
//		String query = "select count(wifi_no) from wifi where wifi_no like ? '%'";
//		int totalPage = 0;
//		
//		try
//		{
...
//			local = local.substring(0, 2);
//		
//		String query = "select * from wifi where wifi_no like ? '%' limit ?, ?";
//	
//		try
//		{
//			pstmt = con.prepareStatement(query);
