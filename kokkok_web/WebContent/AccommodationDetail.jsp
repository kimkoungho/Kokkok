...
<%
	...
	HashMap<String, Object> hash = acd.getAccommodationInfo(acc_no);
	
	SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");

	ArrayList<String> img_list = id.getImageURLList(kind, acc_no);
	int recommend_cnt = rd.getRecommendCount(kind, acc_no);
	int scrap_cnt = mid.getScrapCount(kind, acc_no);
	boolean isRecommend = rd.isRecommend(nickName, kind, acc_no);
	boolean isScrap = mid.isScrap(nickName, kind, acc_no);
	
	ArrayList<HashMap<String, Object>> comment_list = cmd.getDB(kind, acc_no, 1);
%>

<!doctype html>
<html>
<head>
	<title> 쇼핑 세부 정보 </title>
    <!-- 반응형 임포트 -->
...
<body>
	...
	<div id="header">
		<div id="title">
			<span class="title_text">
	        	<%=((String)hash.get("acc_no")).substring(0,2)%> > 
	        	<%=hash.get("name")%>
	        </span>
	    ...
	    <div class="area_text">
	        ...
	        <div class="area_info1">
		        ...
		        <div class="area_info_content">
		            ...
		            <span class="info_insert">숙박</span>
		        </div>
		        ...
		        <div class="area_info_content">
		            <span class="info_dept"> 부대시설 </span>
		            <span class="info_insert"><%=hash.get("sub_intro")%></span>
		        </div>
	    	</div>
    ...
    <iframe src="Comment.jsp?kind=accommodation&item_id=<%=acc_no%>&level=1" id="dynamic" style="display:none" name="dynamic"></iframe>
